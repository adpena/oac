...
def run_ingest_plan(
    source_root: Path, plan: IngestPlan, options: AdapterOptions | None = None
) -> IngestReport:
    ...
    state_path = Path(options.incremental_state_path) if options.incremental_state_path else None
    previous_state: dict[str, str] = {}
    if state_path and state_path.exists():
        try:
            previous_state = json.loads(state_path.read_text(encoding="utf-8")).get(
                "fingerprints", {}
            )
        except Exception:
            report.notes.append("Incremental state file was corrupt; performing full scan.")
    ...
    for rule in plan.rules:
        for path in _expand_patterns(source_root, rule.patterns):
            ...
            for candidate in _parse_candidate(path, source_root, rule):
                ...
                # Use source_path + rule.name as the key for state tracking
                state_key = f"{candidate.source_path}:{rule.name}"
                current_fingerprints[state_key] = candidate_id

                if previous_state.get(state_key) == candidate_id:
                    unchanged_count += 1
                    continue
    ...
    if state_path and not options.dry_run:
        state_path.parent.mkdir(parents=True, exist_ok=True)
        payload = {"target": plan.target, "fingerprints": current_fingerprints}
        state_path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
        report.notes.append(f"Incremental state updated: {state_path.name}")
...
def ingest_report_to_dict(report: IngestReport) -> dict[str, Any]:
    return {
        ...
        "stats": {
            ...
            "unchanged_count": report.stats.unchanged_count,
        },
        "notes": report.notes,
        "candidates": [
            {
...
def _expand_patterns(source_root: Path, patterns: tuple[str, ...]) -> list[Path]:
    if oac_core:
        try:
            paths = oac_core.expand_patterns(str(source_root), list(patterns))
            return sorted([source_root / Path(p) for p in paths])
        except Exception:
            pass
    expanded: list[Path] = []
