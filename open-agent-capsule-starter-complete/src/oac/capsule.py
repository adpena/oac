...
def load_capsule(path: str | Path) -> Capsule:
    ...
    return Capsule(
        root=root,
        manifest=manifest,
        identity=_load_markdown_tree(root / canonical.identity),
        memory=_load_markdown_tree(root / canonical.memory),
        procedures=_load_markdown_tree(root / canonical.procedures),
        skills=_load_skills(root / canonical.skills),
    )


def _load_markdown_tree(root: Path) -> list[MarkdownRecord]:
    if not root.exists():
        return []
    records: list[MarkdownRecord] = []
    for path in sorted(root.rglob("*.md")):
        records.append(parse_markdown_record(path))
    return records


def _load_skills(root: Path) -> list[SkillRecord]:
    if not root.exists():
        return []
    skills: list[SkillRecord] = []
